...
HOME_Y = 350.0
Z_RADIUS = 100.0    # Distance from pitch rotational axis to top of build plate
...
def write_5_axis_gcode(newFile, savedFileName, printSettings, startingPositions, directions, chunk_transform3DList, adhesionList, chunk_shellRingsListList, chunk_optimizedInternalInfills, chunk_optimizedSolidInfills):
    ...
    # Homing of all axes should complete with just G28
    ...
    # Set inital rotations
    openFile.write(f"G1 F{AB_FEEDRATE} {YAW_NAME}{round(directions[0][1], 5)} {PITCH_NAME}{round(directions[0][0], 5)}" + "\n")
    ...
    for key in chunk_transform3DList: # For each chunk
        ...
        for i in range(len(transform3DList)):
            if np.size(transform3DList[i]) == 16:
                transform3DList[i][0, 3] += np.sin(theta) * np.cos(phi) * Z_RADIUS
                transform3DList[i][1, 3] -= np.sin(theta) * np.sin(phi) * Z_RADIUS
        ...
        if key != '0': # If it's not the initial chunk, do some preparation to handle the extra 2 axes
            ...
            if round(ASPEED_Scaled[int(key)], 5) != 0 and round(BSPEED_Scaled[int(key)], 5) != 0:
                ...
                # New code
                # Custom axis names defined at top of file
                openFile.write(f"G0 F{G0Z_FEEDRATE} Z{round(nozzleHeight, 5) + 10.0}; Moving Z axis to clear {YAW_NAME} & {PITCH_NAME} Motion\n")
                openFile.write(f"G0 X{HOME_X} Y{HOME_Y}; Moving Print Head to clear {YAW_NAME} & {PITCH_NAME} Motion" + "\n")
                openFile.write(f"G1 F{round(AB_FEEDRATE, 5)} {YAW_NAME}{round(AMOVE_Degrees[int(key)], 5)} {PITCH_NAME}{round(BMOVE_Degrees[int(key)], 5)} ; Moving axes" + "\n")
                ...
                # New code
                openFile.write(f"G1 F{G0Z_FEEDRATE} Z{round(nozzleHeight, 5) + 10.0}; Moving Z axis to clear {YAW_NAME} & {PITCH_NAME} Motion" + "\n")
                openFile.write(f"G0 X{HOME_X} Y{HOME_Y}; Moving Print Head to clear {YAW_NAME} & {PITCH_NAME} Motion" + "\n")
                openFile.write(f"G1 F{round(BSPEED_Scaled[int(key)], 5)} {PITCH_NAME}{round(BMOVE_Degrees[int(key)], 5)}" + "\n")
            elif round(ASPEED_Scaled[int(key)], 5) != 0 and round(BSPEED_Scaled[int(key)], 5) == 0:
                # Old code
                # openFile.write("G0 F" + str(G0Z_FEEDRATE) + " Z" + str(round(nozzleHeight, 5) + 10.0) + "; Moving Z axis to clear A & B Motion" + "\n")
                # openFile.write('G0 X0.0 Y-175.0' + '; Moving Print Head to clear A & B Motion' + '\n')
                ...
                openFile.write(f"G0 F{G0Z_FEEDRATE} Z{round(nozzleHeight, 5) + 10.0}; Moving Z axis to clear {YAW_NAME} & {PITCH_NAME} Motion" + "\n")
                openFile.write(f"G0 X{HOME_X} Y{HOME_Y}; Moving Print Head to clear {YAW_NAME} & {PITCH_NAME} Motion" + "\n")
                openFile.write(f"G1 F{round(ASPEED_Scaled[int(key)], 5)} {YAW_NAME}{round(AMOVE_Degrees[int(key)], 5)}" + "\n")
            else: # Both ASPEED & BSPEED are zero
                openFile.write(f"; No Change in {YAW_NAME}{PITCH_NAME} Motion Required"+"\n")

            # Old code
            # openFile.write('; A & B Axis Motion Complete'+'\n')

            # New code
            openFile.write(f"; {YAW_NAME} & {PITCH_NAME} Axis Motion Complete" + "\n")

            QA = np.array([[np.cos(phi), -np.sin(phi), 0], [np.sin(phi), np.cos(phi), 0], [0, 0, 1]])

            # Old code, assumes rotation around X axis
            #QB = np.array([[1, 0, 0], [0, np.cos(theta), -np.sin(theta)], [0, np.sin(theta), np.cos(theta)]])

            # New Code, our printer rotates around Y axis
            QB = np.array([[np.cos(theta), 0, np.sin(theta)], [0, 1, 0], [-np.sin(theta), 0, np.cos(theta)]])
    ...
    # New code
    openFile.write(f"G1 F{AB_FEEDRATE} {YAW_NAME}0 {PITCH_NAME}0" + "\n")
